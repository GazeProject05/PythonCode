# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 16:22:18 2019
...
def take_counts(column_):
    ...
    a_3_or = []
    for x,y,z in zip(column_,column_[1:],column_[2:]):
        j = x+y+z
        a_3_or.append(j)
    ...
    for x in set(a_3_or):
        for y in set(column_):
            for j,f,k,g in zip(column_,column_[1:],column_[2:],column_[3:]):
                if x+y == j+f+k+g:
                    xxx_count+=1
    ...
    count_array = count_array.reshape(len(set(a_3_or)),len(set(column_)), order='A')            
    matrix_df = pd.DataFrame(count_array,columns=set(column_), index=set(a_3_or))
    print("**************Count************")
    print(matrix_df) 
...
def combined_trans_matrix(df_list):
    ...
    #**********************Print out Matrix***********************
    excel_file = pd.ExcelWriter('Transition_Probabilities_3_or.xlsx')
    x_matrix.to_excel(excel_file, sheet_name='Sheet1', index=False)
    excel_file.save()
    excel_file.close()
    
    excel_file1 = pd.ExcelWriter('summed_counts_3_or.xlsx')
