# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 14:22:50 2019
...
def take_counts(column_):
    ...
    #column_=str(column_)
    a_1_or = []
    for x in sorted(set(column_)):
        for y in sorted(set(column_)):
            j = x+y
            a_1_or.append(j)
    ...
    for y in sorted(a_1_or):
        for f,k in zip(column_,column_[1:]):
            if y == f+k:
                xxx_count+=1
    ...
    count_array = count_array.reshape(len(set(column_)),len(set(column_)), order='A')            
    matrix_df = pd.DataFrame(count_array,columns=sorted(set(column_)), index=sorted(set(column_)))
    print("**************Count************")
    print(matrix_df) 
...
def combined_trans_matrix(df_list):
    ...
    for col in summed_count:
        for x,(i,row) in zip(summed_count[col], summed_count.iterrows()):
            ...
            if x>0:
                ...
                trans_array = np.append(trans_array,None)
    ...
    #**********************Print out Matrix***********************
    excel_file = pd.ExcelWriter('TP_1or_fold6.xlsx')
    x_matrix.to_excel(excel_file, sheet_name='Sheet1', index=False)
    excel_file.save()
    excel_file.close()
    
    excel_file1 = pd.ExcelWriter('counts_1or_fold6.xlsx')
